# Checkpoint Mirror & Termination Full Script with Single Line Comment Addition long with Publish/Discard for all domains in single script
import requests
import urllib3
import getpass
import json
...
def select_domain():
    ...
    user_input = input("Enter domain code (wish/cele/HO/Br): ").strip().lower()
    selected_domain = domain_map.get(user_input)

    if not selected_domain:
        print("[!] Invalid domain code. Please use one of: wish, cele, HO, Br")
        exit()
...
def login(domain_name):
    ...
    payload = {"user": username, "password": password, "domain": domain_name}
...
if __name__ == "__main__":
    try:
        ...
        login(selected_domain)

        mode = input("\nSelect mode (Termination/Mirror): ").strip().lower()
