...
sid = None
updated_rules = []  # Track modified rules with UID and layer
...
def remove_object_from_rules(object_name, usage_data):
    ...
    if not rules:
        print("[✓] Object is not used in any access rules.")
        return False
    ...
    for ref in rules:
        ...
        for field in ["source", "destination"]:
            ...
            if any(obj.get("type") in valid_types and obj.get("name", "").lower() == object_name.lower() for obj in current_list):
                ...
                update_payload = {"uid": rule_uid, "layer": layer, field: new_list}
                ...
                if update_resp.status_code == 200:
                    ...
                    any_updated = True
                    updated_rules.append({"uid": rule_uid, "layer": layer})
                else:
...
# --- Add Comment to Modified Rules ---
def add_comment_to_rules(comment_text):
    headers = {"X-chkp-sid": sid, "Content-Type": "application/json"}
    for rule in updated_rules:
        ...
        if response.status_code == 200:
            print(f"\n[✓] Added comment to rule {rule['uid']}")
        else:
            print(f"[!] Failed to add comment to rule {rule['layer']}: {response.text}")
...
def publish(object_name):
    ...
    if updated_rules:
        comment_text = input("[?] Enter comment to apply to modified rules: ").strip()
        if comment_text:
            add_comment_to_rules(comment_text)
