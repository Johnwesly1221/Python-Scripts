...
def mirror_diff_add(host_a, host_b):
    ...
    print(f"[→] '{host_b}' is used in {len(rules_b)} rule(s).")

    for ref in rules_a:
        rule_uid = ref["rule"]["uid"]
        layer = ref["layer"]["name"]
        if rule_uid in rule_uids_b:
            continue  # skip if host-B already present
            ...
            if field in list_fields and isinstance(field_value, list):
                if any(obj.get("name", "").lower() == host_a.lower() for obj in field_value):
                    ...
                    if update_resp.status_code == 200:
                        print(f"[✓] Added '{host_b}' to field '{field}' in Rule No: {rule_number}")
...
if __name__ == "__main__":
    try:
        try:
            valid_modes = {"termination": "termination", "mirror": "mirror", "mirror_diff_add": "mirror_diff_add"}
            ...
            if mode == "termination":
                ...
                host_a = input("Enter reference object (host-A): ").strip()
